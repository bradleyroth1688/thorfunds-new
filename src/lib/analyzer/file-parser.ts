...
// Get pdfjs - loaded via CDN script tag on the analyze page
...
export async function parsePDF(arrayBuffer: ArrayBuffer): Promise<ParseResult> {
  try {
    let pdfjsLib = getPdfJs();
    
    // If CDN script hasn't loaded yet, wait a moment and retry
    if (!pdfjsLib) {
      await new Promise(r => setTimeout(r, 1500));
      pdfjsLib = getPdfJs();
    }
    
    // Still not loaded — try loading it now
    if (!pdfjsLib) {
    ...
    if (!pdfjsLib) {
      return { holdings: [], error: "PDF parser couldn't load. Please try uploading a CSV file instead, or enter holdings manually." };
    }
